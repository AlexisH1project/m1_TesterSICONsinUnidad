					...
					<?php 
						...
						if(isset($_POST['buscar'])){
							...
							if( $qnaBuscar != "" && $anioBuscar != ""){
								...
								$sql="SELECT * FROM fomope_qr WHERE ( qna='$qnaBuscar' AND anio='$anioBuscar'  AND tipoRegistro = 'EVENTUALES') ORDER BY id_movimiento_qr DESC";
		
							}elseif ( $qnaBuscar == "" && $anioBuscar == "") {
								
								$sql="SELECT * FROM fomope_qr WHERE ( color_estado ='guinda' AND tipoRegistro = 'EVENTUALES') ORDER BY id_movimiento_qr DESC";
								...
								$sql="SELECT * FROM fomope_qr WHERE ( qna='$qnaBuscar'  AND tipoRegistro = 'EVENTUALES') ORDER BY id_movimiento_qr DESC";
								
							}elseif ( $qnaBuscar == "" && $anioBuscar != "") {
								
								$sql="SELECT * FROM fomope_qr WHERE (anio='$anioBuscar'  AND tipoRegistro = 'EVENTUALES') ORDER BY id_movimiento_qr DESC";
				...
				if($totalFilas == 0){
					...
					while($ver=mysqli_fetch_row($result)){ 
						...
						<tr>
							...
							<td>
							<?php
								if($ver[51] == "negro" || $ver[51] == "negro1"){
									$elEstatus = $ver[49];
								}else{
									$elEstatus = "OPERADO EN QNA".$ver[3]."/".$ver[46];
								}
