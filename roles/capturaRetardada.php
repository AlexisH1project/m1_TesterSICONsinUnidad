...
// ****************************************************** form_fomope
		...
		$(document).ready(function(){
			...
			function irAbandeja() {
				...
				// formulario.action = './capturaRetardada.php?usuario_rol='+id_mov;
				// formulario.submit();
			}
...
<form  enctype="multipart/form-data" id="formDatos" name="captura1" action="" method="POST"> 
					...
					<div class="col-md-9">
						<div class="form-row">
							...
							<div class="form-group col-md-2">
								<label  class="plantilla-label" for="laQna">*QNA: </label>
									<select class="form-control border border-dark custom-select" id="qnaOption" name="qnaOption" required>
										...
										?>
											<option selected="selected" data-subtext="<?php echo $_POST['qnaOption']; ?>"><?php echo $_POST['qnaOption']; ?></option>
									<?php
										}
										?>  
									</select>
							...
							<div class="form-group col-md-2">
								...
								<select class="form-control border border-dark custom-select" name="anio" required>
										 <?php
										...
										if(isset($_POST['anio'])){
										?>
										<option selected="selected" data-subtext="<?php echo $_POST['anio']; ?>"><?php echo $_POST['anio']; ?></option>
										<?php
										}
										?>
				...
				<div class="form-row">
					...
					<div class="form-group col-md-4">
						<label class="plantilla-label" for="codigo">*Código:</label><div class="container">
							 <input onkeypress="return pulsar(event)" type="text" class="form-control border border-dark" id="codigo" name="codigo" placeholder="Ej. 123" value="<?php if(isset($_POST["codigo"])){echo $_POST["codigo"];} ?>" maxlength="9" onkeyup="javascript:this.value=this.value.toUpperCase();" >
						<small name= "alertFechaIngreso" id= "alertFechaIngreso" class="text-danger">
				        </small>  
							</div>
					</div>
				...
				<div class="col-md-8 col-md-offset-8">
					...
					<div class="col">
							...
							<div class="md-form md-0">
								<input type="submit" class="btn btn-primary" id="bandejaEntrada" onclick="irAbandeja()" name="nuevo" style="display: none;" value="nuevo">
								<br>
							<br>
							</div>	
						<?php 
						...
						<div class="md-form md-0">
							<input type="submit" class="btn btn-primary" id="bandejaEntrada"  onclick="irAbandeja()" name="nuevo" value="nuevo">
							<br><br>
						</div>	
						<?php 
						}else{
		...
		//******************************************* */    agregamos los datos personales del empleado si no existe registro de el 
									...
									$dir_subida = './Controller/DOCUMENTOS_MOV/'.$idDoc[1].'/';
											...
											if (move_uploaded_file($_FILES['nameArchivo']['tmp_name'], $fichero_subido)) {
												sleep(3);
												$concatenarNombreC = $dir_subida.strtoupper($rfcAdd."_".$idDoc[1]."_".$apellido1Add."_".$apellido2Add."_".$nombreAdd."_X_".$banderaid."_.".$extencion3);
...
<!-- ***************************************************************************************** -->	
 ...
 <?php
			...
			if(isset($_POST['nuevo'])){
				$usuarioSeguir = $_GET['usuario_rol'];
					//session_destroy();
				echo "<script type='text/javascript'> javascript:window.location='./capturaRetardada.php?usuario_rol=$usuarioSeguir'</script>";  //=$usuarioE
